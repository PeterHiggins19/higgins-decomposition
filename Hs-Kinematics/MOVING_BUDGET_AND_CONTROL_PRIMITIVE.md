...
All realms that conserve‑and‑apportion are compositions, and Hˢ reads them at any scale (the CNQ tiling carries the **exact D=4 chart** reading — IEEE floor — to **D = 10⁶** via a tree atlas, reconstructing to **~4.1e‑12 floating‑point**, not bit‑exact identity). So the same primitive — **read the shape's motion, track the budget's motion, watch their coupling, behind guards and breakers, hash‑receipted, non‑contact** — applies across:
