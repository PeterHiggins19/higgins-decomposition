...
| **L-6 · Field (with Matthew)** | Validate + calibrate on ground-truthed sections; co-author method paper | demo ready → expert-validated correspondence + calibration set | 4 | SYS-003; validation (ConOps S1) | TBD-05 |
...
- **Every field session is a verification event.** When we run the phone + clip-on on a section you know, that test retires real requirements in the VCRM (SYS-003, the ConOps S1 validation) — the same evidence a flight program needs. Nothing is throw-away.
- **Every calibration sets a flight parameter.** The reliability weights and delta-corrections you establish onsite become the instrument's calibration baseline (TBD-05) carried forward — field work *is* flight prep.
...
- **You hold the science.** You are the domain + validation authority throughout (HGS-006 §2); the instrument reports, you interpret. Co-authorship from Stage 1.
- **It stays honest the whole way.** Claim tiers, the ground digital twin (every result reproducible bit-for-bit for your review), and the hash chain mean a skeptical reviewer — or you — can check everything.

The ask is small and concrete: **one working session on a ground-truthed section**, run to this spec, to open Stage 1. The rest of the tree is built and waiting.
