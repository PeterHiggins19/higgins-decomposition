...
| 1 | **Presentation** — **single grayscale PDF, 21 slides, 16:9 widescreen** (numbered N / 21): the talk (standard CoDa → how we add time → the three case studies), then the **rest-of-world finale** (the other six countries), then the **live close — 30 sec PDF flash-through of the CN-TT Output Stage 1 plates (the raw-data provenance, played movie-like) + 30 sec live HTML projector**. ~14 min spoken + ~1 min live close = ~15 min, then 5 min Q&A. White background, black text, hatched (value + pattern) size-view and Power-Share figures for low-ink printing and distance contrast. The talk is named on its subject, **deceptive drift**, defined on the slide where it first appears. **PDF-only delivery** as of 2026-05-28: the previous PPTX has been archived; figures and fonts in this version use the expanded canvas (margins 0.4 in, content 10.2 in × 7.65 in; figures 9.50–10.20 in wide; fonts +20 % for distance reading). Companion: [`SPEAKING_SCRIPT_QA_companion.md`](SPEAKING_SCRIPT_QA_companion.md) · [`.pdf`](SPEAKING_SCRIPT_QA_companion.pdf). | [`data_outputs/CodaWork2026_Presentation_2026-05-28.pdf`](data_outputs/CodaWork2026_Presentation_2026-05-28.pdf). The prior PPTX + PDF (2026-05-27 layout) archived at [`archive/talk_decks_pre_pdfonly_2026-05-28/`](archive/talk_decks_pre_pdfonly_2026-05-28/); the 13-slide colour predecessor (and its scripts) at [`archive/talk_decks_pre_presentation_2026-05-27/`](archive/talk_decks_pre_presentation_2026-05-27/); earlier stages at [`archive/talk_decks_pre_13slide_2026-05-24/`](archive/talk_decks_pre_13slide_2026-05-24/) and [`archive/talk_decks_pre_10slide_2026-05-20/`](archive/talk_decks_pre_10slide_2026-05-20/). |
| 2 | **CN-TT Output — full-corpus raw-data provenance reference** — 325-page master PDF (renamed from PremierDataOutput per HUF-STD-002 Tensor Train I/O Standard): master cover + 9 country sections × 6 plates each (Stage 0 Foundations · Stage 1 Section view · Stage 1 ILR-Helmert Triplet · Stage 2/3 · CNQ). The raw-data provenance for everything downstream — every slide in the Presentation traces back to a plate here, with the per-country JSON / PDF artifacts as the per-step engine output. **The talk's close flashes through this PDF (30 sec, movie-like animation of the Stage 1 plate trajectory) before handing to the live HTML projector (30 sec).** Manual plate-by-plate verification is a HUF-system requirement; this is where a reviewer does it. | [`data_outputs/CodaWork2026_CN-TT_Output_2026-05-28.pdf`](data_outputs/CodaWork2026_CN-TT_Output_2026-05-28.pdf) · source PPTX: [`CodaWork2026_PremierDataOutput_2026-05-13.pptx`](data_outputs/CodaWork2026_PremierDataOutput_2026-05-13.pptx) |
...
├── SPEAKING_SCRIPT_QA_companion.md  ← side-by-side speech + Q&A bench reading aid (21-slide deck)
├── SPEAKING_SCRIPT_QA_companion.pdf ← PDF render of the reading aid (landscape, large speech font)
...
│   ├── CodaWork2026_Presentation_2026-05-28.pdf    ← Piece 1 — THE TALK (single grayscale PDF, 21 slides; 16:9 widescreen; PDF-only as of 2026-05-28)
│   ├── CodaWork2026_CN-TT_Output_2026-05-28.pdf       ← Piece 2 — CN-TT Output, raw-data provenance (325 pp); 30s flash-through at the close
│   ├── CodaWork2026_PremierDataOutput_2026-05-13.pptx ← Piece 2 source PPTX — full-corpus editing source (66 slides; same engine output as the CN-TT PDF)
│   ├── CodaWork2026_PremierDataOutput_2026-05-13.pdf  ← Piece 2 prior-name PDF — superseded by CN-TT_Output_2026-05-28.pdf as the public-face artifact (byte-identical content)
│   ├── codawork2026_projector.html    ← Piece 3 — projector v2.2 (RADAR/BARY/ALIGN + SHOCK); 30s at the close after the CN-TT flash-through
...
└── archive/                           ← superseded material — preserved for lineage
    ...
    ├── talk_decks_pre_pdfonly_2026-05-28/  ← the 21-slide PPTX + PDF as they stood before the 2026-05-28 PDF-only + expanded-layout rebuild (immediate predecessor)
    ├── talk_decks_pre_presentation_2026-05-27/  ← 13-slide colour deck + its scripts + builders
...
1. Open **`data_outputs/CodaWork2026_Presentation_2026-05-28.pdf`** full-screen in any PDF viewer (Acrobat / Preview / Edge / Chrome). Walk through the 21-slide arc (numbered N / 21, ~14 min spoken): title (standard CoDa → adding time to the simplex) → the size view's blind spot, where **deceptive drift** is defined (Germany Solar 2005–2006, Activation Coefficient ≈ 333×) → the method diagram (the five named readings, each defined) → the Activation Coefficient → three archetypes → Germany / Japan / UK each as a pair (share-and-structural-work view, then the trajectory on the simplex) → deceptive drift across the corpus (5 of 9) → the **rest-of-world finale** (the other six: Australia / China / India present, France / United States / World absent) → the close (what the stack answers; hand to the live instrument). The bread analogy is kept on the Activation-Coefficient slide, marked as an analogy. The verbal beat-by-beat + Q&A bench is in [`SPEAKING_SCRIPT_QA_companion.md`](SPEAKING_SCRIPT_QA_companion.md) / [`.pdf`](SPEAKING_SCRIPT_QA_companion.pdf) (large speech font for low-light podium reading).
2. **Slide 21 hands to the live close — two artifacts, ~30 sec each.** First, open **`data_outputs/CodaWork2026_CN-TT_Output_2026-05-28.pdf`** and flash through the Stage 1 plates page-by-page at ~1 sec per page — the data points move on the simplex like frames of a film, showing the raw provenance of every claim made in the talk. Then open **`data_outputs/codawork2026_projector.html`** in a browser. Click a country code, then **BARY** for the trajectory, **ALIGN** for the centred view, **SHOCK** to highlight Aitchison-step shocks. Take questions with the manifold live. Both artifacts run offline; no network required.
3. For any Q&A question that reaches past the trajectories, the **CN-TT Output** (`data_outputs/CodaWork2026_CN-TT_Output_2026-05-28.pdf` — 325 pages, with PPTX editing source at `CodaWork2026_PremierDataOutput_2026-05-13.pptx`) holds all 27 plates per country + Stage 2/3 + CNQ for all nine countries. Manual plate-by-plate verification — a HUF-system requirement — is done here.
...
- **Pre-Presentation deck** (`talk_decks_pre_presentation_2026-05-27/`): the 13-slide colour talk deck (navy/gold) that was active 2026-05-24 → 2026-05-27, with its builders and its two 13-slide speaking-script files. Superseded on 2026-05-27 by the single grayscale 21-slide Presentation (talk + rest-of-world finale + live-projector close; deceptive-drift terminology; hatched grayscale figures; per-slide numbering).
...
- **Legacy speaker prep** (`prep_docs_legacy_2026-05-13/`): SPEAKER_BRIEF, BACKUP_PRESENTATION, CHEAT_SHEET, PEDAGOGICAL_TABLES, QA_BENCH, STUDY_PAGE. These reference the May-13 structure; using them against the active 21-slide deck will mislead.
