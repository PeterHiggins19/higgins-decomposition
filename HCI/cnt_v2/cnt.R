...
SCHEMA_VERSION         <- "2.0.0"     # JSON schema version
ENGINE_VERSION         <- "2.0.2"     # Engine version (added user config block 2.0.1 -> 2.0.2)
ENGINE_NAME            <- "cnt"
...
compute_depth <- function(records_closed, tensor_block, carriers) {
  ...
  } else { ir$damping_zeta <- 0; ir$classification <- "DEGENERATE" }
...
cnt_run <- function(csv_path, output_path = NULL, ordering = NULL) {
  ...
  # Format (v2.0.0 schema)
  json_out <- list(
    metadata = list(
      `_function`    = "provenance",
      `_description` = "Identity, schema version, run-time provenance.",
      schema_version = SCHEMA_VERSION,
      engine_version = paste0(ENGINE_NAME, " ", ENGINE_VERSION),
      engine_implementation = "R",
      generated = format(Sys.time(), "%Y-%m-%dT%H:%M:%OS6Z", tz = "UTC"),
      ...
      engine_config = list(
        `_description`         = paste0("Active values from the USER CONFIGURATION block at top of cnt.R. ",
    ...
    input = list(
      ...
      n_records = length(records), n_carriers = length(carriers),
      carriers = as.list(carriers),
      labels = lapply(records, `[[`, "label"),
      zero_replacement = ing$zero_meta,
      ordering = ordering
    ),
    tensor      = format_tensor_block_v2(tensor_block),
    stages      = list(stage1 = format_stage1_v2(s1),
  ...
  if (!is.null(output_path)) {
    writeLines(toJSON(json_out, auto_unbox = TRUE, pretty = TRUE, digits = NA), output_path)
  }
  json_out
}
...
if (!interactive()) {
  ...
  if (length(args) < 1) {
    cat("Usage: Rscript cnt.R input.csv [output.json] [--temporal] [--ordering-method M]\n")
    quit(status = 1)
  }
  input_path <- args[1]
  output_path <- if (length(args) >= 2 && !startsWith(args[2], "--")) args[2]
                 else paste0(tools::file_path_sans_ext(input_path), "_cnt.json")
  is_temporal <- "--temporal" %in% args
  ord_method  <- "as-given"
  for (i in seq_along(args)) {
    if (args[i] == "--ordering-method" && i < length(args)) ord_method <- args[i + 1]
  }
  ordering <- list(is_temporal = is_temporal, ordering_method = ord_method,
                   caveat = if (!is_temporal) "Order treated as arbitrary; derivative fields ordering-dependent." else NULL)
  cat(sprintf("CNT Engine v%s  (schema %s) — R implementation\n", ENGINE_VERSION, SCHEMA_VERSION))
  cat(sprintf("Input:    %s\n", input_path))
  cat(sprintf("Output:   %s\n", output_path))
  cat(sprintf("Ordering: temporal=%s method=%s\n\n", is_temporal, ord_method))
  j <- cnt_run(input_path, output_path, ordering)
  cat(sprintf("=== Summary ===\n"))
  cat(sprintf("  T = %d, D = %d\n", j$input$n_records, j$input$n_carriers))
  cat(sprintf("  Curvature depth = %d (%s)\n",
              j$depth$summary$curvature_depth,
              j$depth$curvature_tower[[length(j$depth$curvature_tower)]]$status))
  cat(sprintf("  Energy depth    = %d\n", j$depth$summary$energy_depth))
  if (!is.null(j$depth$curvature_attractor$amplitude)) {
    cat(sprintf("  Period-%d attractor: A=%.4f\n",
                j$depth$curvature_attractor$period,
                j$depth$curvature_attractor$amplitude))
  }
  cat(sprintf("  M^2 residual    : %.2e\n", j$depth$involution_proof$mean_residual))
  cat(sprintf("  content_sha256  : %s...\n", substr(j$diagnostics$content_sha256, 1, 16)))
  cat("The instrument reads. The expert decides. The loop stays open.\n")
}
