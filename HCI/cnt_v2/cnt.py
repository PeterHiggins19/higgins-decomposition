...
SCHEMA_VERSION = "2.0.0"        # JSON schema version; see CNT_JSON_SCHEMA.md
ENGINE_VERSION = "2.0.3"        # 2.0.3: IR taxonomy gains ENERGY_STABLE_FIXED_POINT, CURVATURE_VERTEX_FLAT, D2_DEGENERATE classes.
ENGINE_NAME    = "cnt"
...
def cnt_run(csv_path: str,
            ...
            ordering: Optional[dict] = None) -> dict:
    """Top-level CNT run, schema 2.0.0.
    ...
    Returns:
        The full canonical JSON as a dict, conforming to schema 2.0.0
        (every analytic block split into coda_standard / higgins_extensions
        sub-blocks with _function / _description meta).
    """
    t0 = time.time()
    if ordering is None:
        ordering = {
...
def main():
    ...
    if cur.get("amplitude") is not None:
        ...
        print(f"  Period-{cur.get('period','?')} attractor: A={cur.get('amplitude'):.4f}, "
              f"contraction lambda={lyap:.4f}, IR class = {ir['classification']}")
    if eitt["M_sweep"]:
