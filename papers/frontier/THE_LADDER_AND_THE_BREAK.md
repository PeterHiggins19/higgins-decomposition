...
## The complement — the rotation does not break

The break above bounds the *identity* (a composition equal to a single hypercomplex number) at four. It does
**not** bound the *rotation*: `SO(n)` can be generated **exactly for any n** via the `Spin(n)` rotor sandwich
`R v R̃` in `Cl(n)`, whose coordinate form is a product of `⌊n/2⌋` commuting 2-plane rotations — measured exact
to the IEEE floor to **n = 1024** (`HOW_FAR_THE_MATH_GOES.md`; generator
`experiments/son_generator_2026-06/son_exact_generator.py`, receipt `8107b173…`). So the full answer to "how
far does the math go" is two-sided: **rotation, all the way; the single-number identity, to four; arbitrary
dimension, by tiling.**
