...
def generate_codes(payload, comparison=None, shock=None):
    ...
    cg = inp.get("carrier_guard")                              # E-21 carrier guard -> calibration codes
    if cg:
        sz = cg.get("excluded_structural_zero", [])
        if sz: _add(codes, "GD-ZRC-CAL", "CAL",
                    f"{len(sz)} structural-zero carrier(s) excluded — no positive value across all records, undefined in Aitchison geometry: {sz}")
        cc = cg.get("flagged_constant", [])
        if cc: _add(codes, "GD-CNC-CAL", "CAL",
                    f"{len(cc)} constant carrier(s) flagged (admissible, retained): {cc}")
...
def detect_modes(codes):
    ...
    if has("GD-ZRC"):
        modes.append({"mode": "SM-SZC-CAL", "msg": "structural-zero carrier(s) excluded — confirm they are true absences (not missing/unmeasured data) before interpreting the read"})
    if has("L4-RGB"):
