...
def run(csv_path, high_d_threshold=64, atlas_strategy="hierarchical"):
    ...
    health = geo.carrier_health(M)                             # E-21 carrier guard (admissibility triage)
    carrier_guard = None
    if health["structural_zero"] or health["constant"]:
        ...
        if health["structural_zero"]:                          # drop only the undefined carriers; keep order
            keep = health["active"]; M = M[:, keep]; carriers = [carriers[j] for j in keep]
    ...
    if not np.isfinite(clr).all():                             # defense-in-depth: never feed non-finite to eigh
    ...
    if carrier_guard is not None:                              # E-21: only present when carriers were excluded/flagged (clean data -> identical payload+hash)
