...
> **First principle — safety is dominant and absolute.** Hˢ/HUF is **not merely a powerful compositional system**; it is one in which **safety concerns are dominant and absolute, ahead of capability**. The operator holds the last breaker (LOOP‑001 / SAFE‑001): *full automation is never possible, at any scale the operator chooses.* Coherence is **offered, never imposed** — observe‑don't‑impose, operator‑gated, distributed, auditable. Capability is admitted only behind governance; this folder is how that rule is kept. The higher purpose is **constructive coherence with discipline**.
